/**
 * GPU resource manager. Phase 2.5c.
 *
 * ============================================================================
 * OWNERSHIP
 * ============================================================================
 * ENGINE_RECONCILIATION §2.1: identity determines owner. Geometry, textures,
 * and materials are content-addressed, so this manager owns them; the mirror
 * owns nodes. ENGINE_RUNTIME §2.3 adds that ownership (the right to dispose)
 * and accounting (the obligation to report cost) are different
 * responsibilities — this class does both for GPU objects, and reports upward.
 *
 * Content addressing is what makes "duplicate geometry must never allocate
 * twice" achievable: two identical descriptors hash the same, so the second
 * create returns the first handle with an incremented reference count.
 *
 * ============================================================================
 * ON-AIR PINNING
 * ============================================================================
 * ENGINE_RUNTIME §4.4 inverts normal engine behaviour: nothing referenced by
 * an on-air output may be evicted, ever. A game engine evicts to fit; a
 * broadcast engine refuses to load and reports it, because the eviction is
 * visible to an audience. Reference counting alone gives that — a resource with
 * a live reference is never collected — but the budget must therefore be able
 * to refuse.
 */
...
export class GpuResourceManager {
  #nextId = 1;
  readonly geometry: ResourcePool<BufferGeometry>;
  readonly material: ResourcePool<Material>;
  readonly texture: ResourcePool<Texture>;
